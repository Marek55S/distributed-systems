...
public class CalcI implements Calc {
	...
	public long add(int a, int b, Current __current) {
		System.out.println("Using servant: " + __current.id.name);
	...
	public long subtract(int a, int b, Current __current) {
		...
		if (a > 1000 || b > 1000) {
			try {
				...
				Thread.currentThread().interrupt();
			}System.out.println("Using servant: " + __current.id);
		}
	...
	@Override
	public double avg(long[] numbers, Current __current) throws NoInput {
		System.out.println("Using servant: " + __current.id.name);
		if (numbers == null || numbers.length == 0) {
			throw new NoInput("Array is empty");
		}
		long sum = 0;
		for (long n : numbers) {
			sum += n;
